...
tstep       = load([InputDir  'FortranOutputDir/BaselineOutputSubdir/deltatransvec.txt']);
...
Save = lSave;
MonetaryShockSize =NOFS.sticky.mpshock(1,1);
if MonetaryShockSize > 0
   locfigname = ['NegativeMPS_' num2str(abs(MonetaryShockSize)) '_lags' num2str(tlim(2)) '.png'];
else
   locfigname = ['PositiveMPS_' num2str(abs(MonetaryShockSize)) '_lags' num2str(tlim(2)) '.png']; 
end    
...
set(gca,'FontSize',14);
if Save==1
    saveas(gcf,[SaveDir '/fig_3a_' figname locfigname]);
...
legend({'Wealth' 'Income' 'Consumption' 'Illiquid' 'Liquid'},'Location','Best','Interpreter','latex');
% xlim(tlim);
xlim([0 40]);
if MonetaryShockSize > 0
...
%%
NOFS.sticky.fig3rb=400.*(NOFS.sticky.rb-initss.rb);
NOFS.sticky.fig3ra=400.*(NOFS.sticky.ra-initss.ra);
NOFS.sticky.fig3pi=400.*NOFS.sticky.pi;
NOFS.sticky.fig3rnom=400.*(NOFS.sticky.rnom-initss.rnom);
NOFS.sticky.fig3empgap = 100.*log(NOFS.sticky.htot./NOFS.initss.htot);
NOFS.sticky.fig3outgap = 100.*log(NOFS.sticky.output./NOFS.initss.output);
NOFS.sticky.fig3Cgap = 100.*log(NOFS.sticky.Ctot./NOFS.initss.Ctot);
NOFS.sticky.fig3Igap = 100.*log(NOFS.sticky.Itot./NOFS.initss.Itot);
NOFS.sticky.fig3dgap = 100.*log(NOFS.sticky.dtot./NOFS.initss.dtot);
NOFS.sticky.fig3kgap = 100.*log(NOFS.sticky.ktot./NOFS.initss.ktot);
NOFS.sticky.fig3nwg = a;
NOFS.sticky.fig3incg = b;
NOFS.sticky.fig3cong = c;
NOFS.sticky.fig3illg = d;
NOFS.sticky.fig3liqg = e;

load('compare_original.mat')

Dif_fig3.rb = NOFS.sticky.fig3rb - fig3rb;
Dif_fig3.ra = NOFS.sticky.fig3ra - fig3ra;
Dif_fig3.rpi = NOFS.sticky.fig3pi - fig3pi;
Dif_fig3.rnom = NOFS.sticky.fig3rnom - fig3rnom;
Dif_fig3.empgap = NOFS.sticky.fig3empgap - fig3empgap;
Dif_fig3.outgap = NOFS.sticky.fig3outgap - fig3outgap;
Dif_fig3.Cgap = NOFS.sticky.fig3Cgap - fig3Cgap;
Dif_fig3.Igap = NOFS.sticky.fig3Igap -fig3Igap;
Dif_fig3.dgap = NOFS.sticky.fig3dgap-fig3dgap;
Dif_fig3.kgap = NOFS.sticky.fig3kgap - fig3kgap;
Dif_fig3.nwg = NOFS.sticky.fig3nwg - fig3nwg;
Dif_fig3.incg = NOFS.sticky.fig3incg - fig3incg;
Dif_fig3.cong = NOFS.sticky.fig3cong - fig3cong;
Dif_fig3.illg = NOFS.sticky.fig3illg - fig3illg;
Dif_fig3.liqg = NOFS.sticky.fig3liqg - fig3liqg;

nq = 25;
reqd_data=table((1:nq)',Dif_fig3.rb(1:nq,1),...
    Dif_fig3.rpi(1:nq,1),Dif_fig3.rnom(1:nq,1),...
    Dif_fig3.ra(1:nq,1),Dif_fig3.empgap(1:nq,1),...
    Dif_fig3.outgap(1:nq,1),Dif_fig3.Cgap(1:nq,1),...
    Dif_fig3.Igap(1:nq,1),Dif_fig3.dgap(1:nq,1),Dif_fig3.kgap(1:nq,1),...
    Dif_fig3.nwg(1:nq,1), Dif_fig3.incg(1:nq,1),Dif_fig3.cong(1:nq,1),...
    Dif_fig3.illg(1:nq,1),Dif_fig3.liqg(1:nq,1));
 
reqd_data.Properties.VariableNames=...
    {'Quarter','Real Liq','Inflation','Nominal Liq',...
    'Real Ill','Employment Gap','Output Gap','Consumption Gap',...
    'Inv Gap','Dep Gap','Cap Gap','NW-Gini','Inc-Gini',...
    'Con-Gini','Illiq-Gini','Liq-Gini'};
 
writetable(reqd_data,strcat('Fig3_Differences_',int2str(nq),'_quarters.xlsx'),'Sheet',1);   
...
set(leg,...
    'Position',[0.516574479837412 0.277100083612826 0.300092186829255 0.254645950102288],...
    'Interpreter','latex');
xlim(tlim);
%ylim([-0.1 0.5]);
ylabel('Deviation (\%)', 'interpreter','latex','FontSize',14);
...
set(gca,'FontSize',14);
if Save==1
    saveas(gcf,[SaveDir '/fig_4b_' figname locfigname]);
